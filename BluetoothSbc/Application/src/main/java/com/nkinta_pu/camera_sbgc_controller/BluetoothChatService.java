...
public class BluetoothChatService {
    ...
    enum MessageReadStatus { START, COMMANDID, DATASIZE, HEADERCHECKSUM, BODYDATA, BODYCHECKSUM, UNKNOWN};
    ...
    private class ConnectedThread extends Thread {
        ...
        private void analyzeMessage(byte[] buffer, int bytes) {

            for (int i = 0; i < bytes; ++i) {
                byte tempByte = buffer[i];
                if (mStatus == MessageReadStatus.START) {
                    if (tempByte == 0x3E) {
                        mStatus = MessageReadStatus.COMMANDID;
                    }
                }
                else if (mStatus == MessageReadStatus.COMMANDID) {
                    mCommandId = tempByte & 0xFF;
                    mTotalHeader += mCommandId;
                    mStatus = MessageReadStatus.DATASIZE;
                }
                else if (mStatus == MessageReadStatus.DATASIZE) {
                    mDataSize = tempByte & 0xFF;
                    mTotalHeader += mDataSize;
                    mStatus = MessageReadStatus.HEADERCHECKSUM;
                }
                else if (mStatus == MessageReadStatus.HEADERCHECKSUM) {
                    if ((mTotalHeader & 0xFF) == (int) (tempByte & 0xFF)) {
                        mStatus = MessageReadStatus.BODYDATA;
                    }
                }
                else if (mStatus == MessageReadStatus.BODYDATA) {
                    if (mDataCount < mDataSize) {
                        mData[mDataCount] = tempByte;
                        mTotalData += tempByte & 0xFF;
                        mDataCount++;
                        if (mDataCount == mDataSize) {
                            mStatus = MessageReadStatus.BODYCHECKSUM;
                        }
                        else {
